...
2020  CALL GETIN(TWOWDS,A,WD2,B)
      K=70
C      IF(A.EQ.'ENTER'.AND.(WD2.EQ.'STREA'.OR.WD2.EQ.'WATER'))GOTO 2010
      IF(CHAR(A).EQ.'ENTER'.AND.
     1 (CHAR(WD2).EQ.'STREA'.OR.CHAR(WD2).EQ.'WATER'))GOTO 2010
C      IF(A.EQ.'ENTER'.AND.TWOWDS.NE.0)GOTO 2012
      IF(CHAR(A).EQ.'ENTER'.AND.TWOWDS.NE.0)GOTO 2012
C 2021  IF(A.NE.'WEST')GOTO 2023
2021  IF(CHAR(A).NE.'WEST')GOTO 2023
      IWEST=IWEST+1
...
5312  CONTINUE
      JOBJ=IOBJ(J)
      GOTO 2027
C 5062  IF(B.NE.' ')GOTO 5333
5062  IF(CHAR(B).NE.' ')GOTO 5333
      PRINT 5063,A
5063  FORMAT('  ',A5,' WHAT?',/)
      GOTO 2020
...
C 502   IF(B.NE.' ')GOTO 5316
502   IF(CHAR(B).NE.' ')GOTO 5316
      PRINT 5005,A
...
C 5064  IF(B.NE.' ')GOTO 5314
5064  IF(CHAR(B).NE.' ')GOTO 5314
      PRINT 5001,A
...
1     FORMAT(4A5)
      ...
      DO 2 J=1,4
      DO 2 K=1,5
      MASK1=O"774000000000"
      IF(K.NE.1) MASK1=O"177"*M2(K)
      IF(IAND(IEOR(A(J),O"201004020100"),MASK1).EQ.0)GOTO 3
      IF(S.EQ.0) GOTO 2
      TWOW=1
      CALL SHIFT(A(J),7*(K-1),XX)
      CALL SHIFT(A(J+1),7*(K-6),YY)
      MASK=-M2(6-K)
      C=IAND(XX,MASK)+IAND(YY,(-2-MASK))
      GOTO 4
3     IF(S.EQ.1) GOTO 2
      S=1
      IF(J.EQ.1) B=IOR(IAND(B,-M2(K)),IAND(O"201004020100",
!C     1 (-M2(K).XOR.-1))
     1 IEOR(-M2(K),-1)))
2     CONTINUE
4     D=A(2)
      ...
      IF(CHAR(IA1).EQ.'NO'.OR.CHAR(IA1).EQ.'N') GOTO 1
      YEA=1
...
10    IDIST=-DIST
      DO 11 I=1,IDIST
      J = 0
      IF (RES.LT.0) J=O"200000000000"
11    RES = (IAND(RES,O"377777777777")/2) + J
20    RETURN
30    DO 31 I=1,DIST
      j = 0
      IF (IAND(RES,O"200000000000").NE.0) J=O"400000000000"
31    RES = IAND(RES,O"177777777777")*2 + J
      RETURN
      END
